...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        today_total_deal_value = (
            today_orders.aggregate(total=Sum("deal_value"))["total"] or 0
        )
        all_order_total_deal_value = (
            order.aggregate(total=Sum("deal_value"))["total"] or 0
        )

        # Fetch todos assigned to the current user
        todos = Todo.objects.filter(work_assign=self.request.user).order_by(
            "-create_date"
        )
        ...
        order_status_counts = (
            order.values("status").annotate(count=Count("id")).order_by("status")
        )
        order_status_counts_dict = {
            item["status"]: item["count"] for item in order_status_counts
        }
        request_status_counts = (
            order_request.values("status")
            .annotate(count=Count("id"))
            .order_by("status")
        )
        request_status_counts_dict = {
            item["status"]: item["count"] for item in request_status_counts
        }
...
class ProductDeleteView(LoginRequiredMixin, DeleteView):
    model = Product
    template_name = (
        "product/product_confirm_delete.html"  # Template for delete confirmation
    )
    success_url = reverse_lazy("product_list")
...
class MaintenanceCostDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        daily_profit = Daily_Profit.objects.get(
            date=timezone.localtime(maintenance_cost.create_date).date()
        )
        print(daily_profit)
...
class DailyProfitListView(ListView):
    ...
    def get_queryset(self):
        ...
        if search_query:
            queryset = queryset.filter(
                Q(note__icontains=search_query) | Q(date__icontains=search_query)
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["total_sell_amount"] = (
            queryset.aggregate(Sum("total_sell"))["total_sell__sum"] or 0
        )
...
class TodoListView(ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "There was an error adding the Todo. Please check the form and try again.",
            )
...
class TodoCreateView(CreateView):
    ...
    def form_valid(self, form):
        form.instance.last_update_user = (
            self.request.user
        )  # Set last_update_user to the current user
        return super().form_valid(form)
...
class TodoUpdateView(UpdateView):
    ...
    def form_valid(self, form):
        form.instance.last_update_user = (
            self.request.user
        )  # Update last_update_user on edit
        return super().form_valid(form)
    ...
    def get_product_summary(self, orders):
        ...
        for order in orders:
            for item in order.order_item.all():
                ...
                product_summary[product_id]["total_quantity"] += qty
